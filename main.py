...
class FilePaths:
    ...
    fnCharList = 'model/ma.txt'
    fnAccuracy = 'model/accuracy.txt'
    fnTrain = 'data/'
    fnInfer = 'data/train/23880.jpg'
    fnCorpus = 'data/mlvocab.txt'
...
def getword(fnImg):
    f = open('data/full.txt', 'r')
    
    lines = f.readlines()
    lines = [x.strip() for x in lines]
    for line in lines:
        linesplit = line.split(' ')
        if 'data/'+linesplit[0] == fnImg:
            return linesplit[1]
...
def infer(model, fnImg):
    ...
    print('originalword: ', getword(fnImg))
